# AI Agent Context — scopeguard.xyz

## Summary

*one paragraph: what this site is, what it does*

Enterprises install AI plugins faster than security can review them. Scopeguard shifts control from the employee to the security team by scanning each tool's requested permissions before install, blocking non-compliant plugins, and integrating into existing browser and identity governance systems. The result: visibility without friction, policy without paranoia.

## Audience

*one sentence: who this is for (broad demographic)*

Security leaders (CISOs, security engineers) managing shadow AI adoption across their workforce.
...
Mid-market to enterprise (500–50k employees) with active AI adoption, a CASB or browser management system in place, and a compliance mandate (SOC 2, FedRAMP, industry-specific data handling rules). They've already seen employees install ChatGPT, Claude, or Copilot extensions without approval and need a lightweight enforcement layer that doesn't require app wrapping or network tunneling.

## Goals

*1-2 sentences: primary business / product goal*

Establish Scopeguard as the standard control point for AI plugin governance in browser and OAuth app management. Become the reference tool CISOs recommend during security reviews and tool audits.
...
`proj=scopeguard.xyz`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=scopeguard.xyz` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=scopeguard.xyz` | Start the dev server, skipping install. |
| `make test proj=scopeguard.xyz` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=scopeguard.xyz
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy scopeguard.xyz"` runs `gh repo create` and
...
- **Live URL:** https://scopeguard.xyz/  *(update once first deploy succeeds)*
...
Lead with technical clarity: explain what OAuth scopes mean, why they matter for data governance, and how Scopeguard makes them visible. Publish case studies of real policy decisions (e.g., "Why we block plugins that request full mailbox read"). Build credibility through integration guides (Chrome MDM, Workspace, Microsoft 365) and compliance documentation (how Scopeguard supports SOC 2, FedRAMP audits). Avoid marketing jargon; speak like a security peer.

### Post-deploy checklist (do these once after the first successful deploy)

- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:scopeguard.xyz` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://scopeguard.xyz/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status scopeguard.xyz` resolves cleanly).
...
`portfolio project check scopeguard.xyz` (run from `sites/portfolio/`).
